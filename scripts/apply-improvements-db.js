...
async function main() {
  ...
  if (!password) {
    console.log('\n============================================================');
    console.log('NBOS ERP IMPROVEMENTS DATABASE MIGRATION SCRIPT');
    console.log('============================================================');
    ...
    console.log(`\n👉 File: ${path.resolve(__dirname, '../supabase/nbos_erp_improvements.sql')}`);
    console.log('\nOPTION B (Direct CLI execution):');
    console.log('Run this script again providing your Supabase database password:');
    console.log('  node scripts/apply-improvements-db.js <your_database_password>');
    console.log('============================================================\n');
    return;
  }
  ...
  const schemaSql = fs.readFileSync(path.join(__dirname, '../supabase/nbos_erp_improvements.sql'), 'utf8');
  ...
  try {
    ...
    console.log('Successfully connected! Applying ERP SQL migrations...');

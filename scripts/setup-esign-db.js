...
async function main() {
  ...
  if (!password) {
    ...
    console.log(`\n👉 File: ${path.resolve(__dirname, '../supabase/esign_schema.sql')}`);
    console.log('\nOPTION B (Direct CLI execution):');
    console.log('Run this script again providing your Supabase database password:');
    console.log('  node scripts/setup-esign-db.js <your_database_password>');
    console.log('============================================================\n');
    return;
  }
  ...
  const schemaSql = fs.readFileSync(path.join(__dirname, '../supabase/esign_schema.sql'), 'utf8');

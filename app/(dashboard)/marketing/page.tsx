...
export default function MarketingIntelligencePage() {
  ...
  const handleUploadDocumentFile = async (files: File[]) => {
    if (!detailReport || files.length === 0) return
    const file = files[0]
    toast({ title: 'Uploading file...', description: file.name })
    try {
      const formData = new FormData()
      formData.append('file', file)
      formData.append('projectId', detailReport.id)
      ...
      const newHistoryEntry = {
        ...
        action: `Report Uploaded: ${file.name}`,
        canDownload: true,
      ...
      const updatedHistory = [...(detailReport.history || []), newHistoryEntry]
      
      if (isSupabaseConfigured()) {
        const { error } = await supabase
          .from('marketing_reports')
          .update({ history: updatedHistory })
          .eq('id', detailReport.id)
        if (error) throw error
      }

      // Update local state
      const updatedReport = { ...detailReport, history: updatedHistory }
      setDetailReport(updatedReport)
      
      const updatedList = reports.map(r => r.id === detailReport.id ? updatedReport : r)
      setReports(updatedList)
      setCachedData('marketing_reports', updatedList)

      toast({ title: 'Success', description: 'Report uploaded and saved successfully!' })
    } catch (e: any) {
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="documents" className="mt-4 space-y-4">
              <FileUpload 
                ...
                multiple={false} 
                label="Upload Generated Report" 
                description="Upload the Excel or PDF report generated by Claude" 
                onFilesSelected={handleUploadDocumentFile} 
              />
              <div className="space-y-2 mt-4">
                <h4 className="text-xs font-bold text-gold uppercase">Uploaded Reports</h4>
                {detailReport?.history && detailReport.history.filter(h => h.canDownload).length > 0 ? (
                  <div className="border border-border rounded-lg divide-y divide-[#152e23] bg-card">
                    {detailReport.history.filter(h => h.canDownload).map((h: any, idx: number) => (
                  ...
                  <p className="text-xs text-muted-foreground py-4 text-center">No reports uploaded yet.</p>
                )}
              </div>
            </TabsContent>
            ...
            <TabsContent value="activity" className="mt-4">
              <VersionTimeline 
                versions={(detailReport?.history || []).map((h, i) => ({ 
                  version: i + 1, 
                  date: h.date, 
                  action: h.action, 
            ...
            <TabsContent value="versions" className="mt-4">
              <VersionTimeline 
                versions={(detailReport?.history || []).filter(h => h.canDownload).map((h, i) => ({ 
                  version: i + 1, 
                  date: h.date, 
                  action: h.action, 
                  canDownload: true, 

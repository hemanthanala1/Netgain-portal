          ...
          <Card className="bg-muted/10 border-border/60">
            ...
            <UniversalTimeline
              ...
              entries={logs.map(log => ({
                action: `${log.channel === 'email' ? '📧 Email' : log.channel === 'whatsapp' ? '💬 WhatsApp' : '📱 SMS'} dispatch: ${log.subject || log.message.substring(0, 55) + '...'}`,
                actionType: log.status === 'failed' ? 'rejected' : 'sent',
                by: 'Staff Member',
                date: log.timestamp,
                status: log.status,
                comment: log.message,
